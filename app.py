...
# 6. ÚJ: ÁLTALÁNOS CSEVEGŐ (TRADING ASSZISZTENS)
@bot.message_handler(func=lambda m: m.text and not m.text.startswith('/'))
def handle_trading_chat(message):
    if not is_authorized(message): return
    
    # Ha a felhasználó egy win/loss választ adott, azt már a fenti handler lekezelte, 
    # de ha véletlenül ide esik be, akkor figyelmen kívül hagyjuk.
    if message.reply_to_message and message.text.lower() in ['win', 'won', 'lost', 'loss']:
        return
